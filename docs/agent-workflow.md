...
- 현재 이미지를 기준 캔버스로 선택
- 이전 정상 이미지와 diff 시각화 이미지의 크기를 현재 이미지 기준으로 통일
- 이전 정상 이미지와 현재 이미지 간 SIFT 특징점 추출
- Lowe ratio test 기반 good match 선택
- RANSAC homography 추정
- 이전 정상 이미지에만 정렬 변환 적용
- 정렬 후 얻은 비중첩 영역 마스크를 이전 정상 이미지와 현재 이미지에 동일 적용
- diff 시각화 이미지는 정렬 변환 없이 크기 통일 후 동일 마스크 적용
- 정렬 품질 평가
...
1. 현재 이미지 기준으로 정렬된 이전 정상 이미지
2. 동일 마스크가 적용된 현재 이미지
3. 크기 통일 및 동일 마스크가 적용된 1차 이상 시각화 이미지
